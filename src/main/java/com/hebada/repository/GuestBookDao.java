...
public class GuestBookDao {
	...
	public PageResults<GuestBook> findByPage(int page , int row){
		...
		if(beginIndex<10000){
			...
			hql = "from GuestBook where not exists " +
					"(select id from  GuestBook order by create_time desc limit :beginIdnex,1) " +
					"order by create_time desc limit :row";
		}
		String hqlCount = "select count(*) from GuestBook";
		//Map map = new HashMap();
		PageResults results = baseDao.findPage(hql, hqlCount, null, page, row);
